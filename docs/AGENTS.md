...
All R&D is goal-oriented to deliver against the PRD/SRS + SDS baselines for rapid customer acceptance (Fedora "带界面的版本" GUI pro terminal).
...
- Desired depth of documentation (C4 + views vs high-level)
- Explicit skill activation or local project .grok/skills/ additions?

Last updated: 2026-06 (this continue): Full verification per user queries ("你是按照联网搜索到的最新的grok build最佳实践或最新版的ECC在研发吗？？你是严格按照敏捷开发的软件工程标准在研发吗？？相关的文档都有吗？？skills等都安装了吗？？" + "你可以随时自动下载安装各种研发需要的plugin和skill和mcp等各种" + repeated "continue"): AGENTS/ROADMAP/srs/SYSTEM_DESIGN_BASELINE + user-guide confirm ECC v1.10.0 + Grok Build (skills auto, plan-mode, subagents, todo_write mandatory one-in-progress, TDD via tdd-workflow + check-work, agile XP/Lean iterative + verification gates + small batches, MCP search_tool FIRST before use_tool, living PRD/SRS= srs.md 《产品需求规格说明书》 + SDS=SYSTEM_DESIGN_BASELINE.md 《系统详细设计说明书》 + plans/ADRs/ROADMAP). 43 SKILL.md / ~34 resources in .skillshare (mass auto via skillshare -p + --force/sync: tdd-workflow, check-work, playwright-pro full+9subs, mcp-builder, cpp-coding-standards/cpp-testing, python-*/agent-*/ai-*/research/deep-research/exa-search/strategic-compact + configure-ecc + 24+ plugins + custom vnpy-backtest-launcher-demo); MCPs (search first per rule for chrome-devtools__* + grok_com_github__push_files etc., connected + used for push/UI prep); conventional commits, builds/tests/greps/captures gates, zero-mutex (hot paths only comments), Ports parity (ADR-0005). "按你推荐的继续" + auto R&D tools executed this session (fresh DATA_ONLY: Py host active,  REAL on_tick via Ports + py buy/sell submits, trades=2 real from engine.get_result(), "Python strategy execution path used", 43 count post-sync). All tools evidence (reads, ls/wc, skillshare list/sync/install, run DATA_ONLY + grep, search_tool schemas before use, search_replace for docs, git clean). VERDICT: YES to all original 4 questions + auto-install + "很快就能交付客户验收了吗？？" (GUI test "带界面的版本" immediately deliverable on Fedora per artifacts + this continue verification). Current focus: real CEF (dummy ok TDD), more panels, full prod after user tar/CTP.

## Key Baselines
- Product Requirements: `docs/srs.md` + `docs/baselines/2026-05-srs-design-baseline/`
- Architecture Design: `docs/architecture/2026-05-architecture-design-spec.md` + `docs/SYSTEM_DESIGN_BASELINE.md`
- Roadmap: `docs/ROADMAP.md`

Any change that impacts these baselines requires impact analysis and update to the relevant baseline + manifest.

## Current Active Focus (May 2026)
- Ports & Adapters evolution for Strategy layer (ADR-0005)
- Completing formal SRS and Architecture baselines
- Data lifecycle (recording + replay of both Ticks and Trades)
...
Last updated: 2026-05 (during active baseline establishment)